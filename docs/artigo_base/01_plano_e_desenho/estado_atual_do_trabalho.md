...
| **Ausência de validação humana da escala de hostilidade** | A escala ordinal 0–4, embora ancorada em distinções conceituais consolidadas na literatura (Papacharissi, 2004; Coe et al., 2014; Rossini, 2022; Bentivegna & Rega, 2022), não foi submetida a processo formal de validação com anotadores humanos neste estudo. Isso implica que: (i) a concordância inter-anotador (*Krippendorff's α* ordinal) não foi mensurada, de modo que não há evidência empírica de que os descritores dos cinco níveis produzem classificações consistentes entre avaliadores independentes; e (ii) a dependência cultural da percepção de hostilidade — reconhecida na literatura como fator significativo (Papacharissi, 2004; Rossini, 2022) — não foi controlada, e a escala pode não generalizar para contextos discursivos distintos do debate político brasileiro simulado neste trabalho. Os resultados de hostilidade reportados devem, portanto, ser interpretados como classificações instrumentais do agente LLM, e não como medidas validadas de hostilidade percebida por humanos. Ver Seção 11 (trabalho futuro) |
| **Ausência de calibração do Juiz C contra ground truth humano** | O Juiz C (agente LLM que classifica hostilidade de forma cega à condição experimental) não foi comparado com classificações humanas de referência. Sem essa calibração, não é possível determinar: (i) se o Juiz C apresenta vieses sistemáticos (e.g., sub-classificar ironia contextual brasileira por ter sido treinado predominantemente em inglês, ou comprimir a escala em torno dos níveis intermediários); (ii) se a concordância entre o Juiz C e avaliadores humanos atinge patamares aceitáveis (*Krippendorff's α* ordinal ≥ 0,667); nem (iii) se a variância intra-juiz (entre as 3 execuções por mensagem) é comparável à variância inter-anotador humana. A ausência dessa calibração é uma limitação direta dos achados quantitativos do estudo: os scores de hostilidade reportados refletem a classificação do LLM, cuja fidedignidade em relação ao julgamento humano não foi estabelecida neste trabalho. Ver Seção 11 (trabalho futuro) |
...
**Validação humana da escala de hostilidade 0–4.** Desenvolver e executar um
protocolo formal de validação da escala com anotadores humanos, estruturado da
seguinte forma:

*Corpus de anotação:* 100–200 mensagens de debate político em português
brasileiro (mistas: geradas pela simulação e coletadas de fóruns/redes sociais
sobre os mesmos temas), com amostragem estratificada para garantir cobertura
dos cinco níveis da escala.

*Anotadores:* 3–5 avaliadores independentes (estudantes de pós-graduação em
comunicação, ciência política ou linguística), treinados com codebook
desenvolvido a partir dos descritores da escala.

*Codebook:* documento autocontido com definição de cada nível (0–4), 2–3
exemplos ancorados por nível no registro do debate político brasileiro online,
instruções sobre o que **não** constitui hostilidade (posição ideológica
extrema sem ataque pessoal) e regras de desempate. O codebook deve ser sensível
ao contexto cultural: expressões que seriam classificadas diferentemente em
outros contextos discursivos devem ser explicitamente endereçadas
(Papacharissi, 2004; Rossini, 2022).

*Processo:* sessão de calibração inicial (anotadores discutem 10 exemplos para
alinhar interpretação, sem inclusão no cálculo de concordância) → anotação
independente → cálculo de *Krippendorff's α* ordinal (Krippendorff, 2011) →
rodada de resolução de discordâncias para produção de rótulos de consenso
(ground truth para a calibração do Juiz C).

*Critério de aceite:* *Krippendorff's α* ordinal ≥ 0,667 (concordância
moderada, mínimo publicável) como patamar mínimo; α ≥ 0,8 (concordância forte)
como meta desejável.

Este protocolo pode ser executado em escopo progressivo: versão mínima com 100
mensagens e 3 anotadores para artigo em conferência; versão completa com
200–400 mensagens, 5 anotadores em duas rodadas, e análise de sensibilidade
cultural para a dissertação de mestrado.

**Calibração do Juiz C contra ground truth humano.** Após a validação da escala
(item anterior), calibrar o Juiz C comparando suas classificações com os
rótulos de consenso humano, usando:

*Concordância LLM-como-anotador:* incluir o Juiz C como anotador adicional no
cálculo de *Krippendorff's α* ordinal e verificar se sua inclusão mantém ou
degrada o coeficiente global.

*Design leave-one-rater-out:* para cada anotador humano, calcular seu desvio
médio em relação ao consenso dos demais; comparar com o desvio médio do Juiz C
em relação ao mesmo consenso. Se o desvio do Juiz C for menor ou igual ao
desvio humano médio, o juiz é comparável a um avaliador humano individual.

*Matriz de confusão 5×5:* identificar vieses sistemáticos do Juiz C — em
especial, se sub-classifica ironia ou sarcasmo contextual do português
brasileiro, e se comprime a escala em torno de níveis intermediários.

*Análise de consistência intra-juiz (test-retest):* submeter as mesmas
mensagens ao Juiz C em momentos distintos para mensurar a variância das
classificações com *temperature* > 0, verificando a estabilidade do
instrumento.

*Baseline automatizado (opcional):* correlacionar os scores da escala 0–4
(humanos e Juiz C) com o score de toxicidade do Perspective API
(Jigsaw/Google) para posicionar o instrumento em relação ao estado da arte
comercial.

*Análise de sensibilidade do limiar:* replicar a análise principal com limiar
de intervenção ≥ 3 (em vez de ≥ 2) para avaliar a robustez dos resultados à
escolha do ponto de corte.

Assim como a validação da escala, este protocolo admite execução progressiva:
comparação básica (α + matriz de confusão) para artigo em conferência;
protocolo completo (leave-one-rater-out, test-retest, baseline Perspective API,
sensibilidade do limiar) para dissertação.
